  ...
  onSubmit() {
    ...
    this.authService.register(userData).subscribe({
      next: (response) => {
        ...
        this.carrito.crearCarrito(response.user.id, estadoCarrito).subscribe({
          next: () => {
            console.log('Carrito creado correctamente', response.user.id);
            this.router.navigate(['/login']);
          },
          error: (err) => {
            console.error('Error al crear el carrito:', err);
            this.router.navigate(['/login']); // navega igual si falla el carrito
          }
        });
